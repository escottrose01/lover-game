...
public class PlatformerController : RaycastController
{
    ...
    public void Move(Vector2 moveAmount, Vector2 input, bool standingOnPlatform = false)
    {
        ...
        if (moveAmount.y < 0f) DescendSlope(ref moveAmount);
        if (moveAmount.x != 0) collisions.faceDirection = (int)Mathf.Sign(moveAmount.x);
    ...
    void ClimbSlope(ref Vector2 moveAmount, float slopeAngle, Vector2 slopeNormal)
    {
        ...
        {
            moveAmount.x = moveDistance * Mathf.Sign(moveAmount.x) * Mathf.Cos(slopeAngle * Mathf.Deg2Rad);
            ...
            if (hit)
            {
                ...
                if (slopeAngle != 0f && slopeAngle <= maxSlopeAngle)
                {
                    if (Mathf.Sign(hit.normal.x) == directionX)
                    {
                        ...
                        {
                            float moveDistance = Mathf.Abs(moveAmount.x);
                            float descendVelocityX = moveDistance * Mathf.Sign(moveAmount.x) * Mathf.Cos(slopeAngle * Mathf.Deg2Rad);
                            float descendVelocityY = moveDistance * Mathf.Sin(slopeAngle * Mathf.Deg2Rad);
                            moveAmount.x = descendVelocityX;
